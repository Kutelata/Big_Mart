...
<?php
include_once '../services/api/ProviderRepository.php';
...
if ($nameValue) {
    foreach ($providerList as $key => $provider) {
        if (strpos($provider['name'], $nameValue) === false) {
...
if ($statusValue) {
    foreach ($providerList as $key => $provider) {
        if ($provider['status'] != $statusValue) {
            unset($providerList[$key]);
        }
    }
}
if ($emailValue) {
    foreach ($providerList as $key => $provider) {
        if (strpos($provider['email'], $emailValue) === false) {
            unset($providerList[$key]);
        }
    }
}
...
if ($phoneValue) {
    foreach ($providerList as $key => $provider) {
        if (strpos($provider['phone'], $phoneValue) === false) {
            unset($providerList[$key]);
        }
    }
}
$providerCount = count($providerList);
$itemPerPage = isset($_GET['item']) ? (int)$_GET['item'] : 5;
$page = ceil($providerCount / $itemPerPage);
$currentPage = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$currentPage = $currentPage > $page ? 1 : $currentPage;
$offSet = $itemPerPage * ($currentPage - 1);

if ($providerCount > $itemPerPage) {
    $providerList = array_slice($providerList, $offSet, $itemPerPage, true);
}
$prePage = $currentPage > 1 ? $currentPage - 1 : false;
$nextPage = $itemPerPage < $providerCount && $currentPage < $page ? $currentPage + 1 : false;
?>
        ...
        <div class="layout-page">
            ...
            <div class="content-wrapper">
                <div class="container-xxl flex-grow-1 container-p-y">
                    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Catalog /</span> Provider
                        <a href="create-provider.php" class="btn btn-primary">Create Provider</a>
                    </h4>
                    <div class="card">
                        <h5 class="card-header"><?= $providerCount . ' records found' ?></h5>
                        <div class="container">
                            ...
                            <div class="input-group" style="width: 100px">
                                ...
                                <div class="dropdown">
                                    <button class="btn btn-secondary dropdown-toggle" type="button"
                                            id="dropdownItemPerPage" data-bs-toggle="dropdown"></button>
                        ...
                        <div class="container">
                            <div class="input-group" style="width: 70px">
                                <?php if ($prePage) { ?>
                                    <a id="pre-page" data-value="<?= $prePage ?>" class="btn btn-danger"><</a>
                                <?php } ?>
                                <input type="text" class="form-control" value="<?= $currentPage ?>" id="page">
                                <?= 'of ' . $page ?>
                                <?php if ($nextPage) { ?>
                                    <a id="next-page" data-value="<?= $nextPage ?>" class="btn btn-danger">></a>
                                <?php } ?>
                            </div>
                        </div>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="../services/controller/ProviderController.php?action=getById&id=<?= $provider['id'] ?>">
                                                        <i class="bx bx-edit-alt me-1"></i> Edit</a>
                                                    <a class="dropdown-item"
                                                       href="../services/controller/ProviderController.php?action=delete&id=<?= $provider['id'] ?>">
